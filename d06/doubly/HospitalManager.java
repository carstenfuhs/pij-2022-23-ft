...
public class HospitalManager {
    ...
    public boolean delete(String name) {
        ...
        if (firstPatient.getName().equals(name)) {
            ...
            // new first patient needs to know that they
            // have no previous patient
        ...
        while (current.getNextPatient() != null) {
            if (current.getNextPatient().getName().equals(name)) {
                ...
                // the previous patient of the deleted one (i.e., current) and
                // the next patient of the deleted one (i.e., newNext)
                // both must be updated
                current.setNextPatient(newNext);
                if (newNext != null) {
                    newNext.setPreviousPatient(current);
		}
                return true;
            }
            current = current.getNextPatient();
